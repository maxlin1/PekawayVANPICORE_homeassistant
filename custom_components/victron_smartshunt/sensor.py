...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    """Setup der VSmart-Sensoren asynchron."""
    port = config["port"]
    ...
    sensors = [VSmartSensor(key, details) for key, details in key_mapping.items()]
    async_add_entities(sensors)
    _LOGGER.info("VSmart-Sensoren wurden initialisiert und hinzugefügt.")
...
class VSmartSensor(Entity):
    """Repräsentiert einen VSmart-Sensor."""
    ...
    def __init__(self, key, details):
        ...
        self._attr_unique_id = f"vsmart_{self._key}"
